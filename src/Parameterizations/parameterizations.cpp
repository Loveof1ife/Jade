...
double Cal_Area(PolyMesh *pMesh)
{
    auto f_it = pMesh->polyfaces_begin();
    MPolyFace* f_start = (*f_it);
    MPolyFace* f_current = f_start;
    double area{0.0};
...
std::vector<Eigen::Matrix2d> Cal_Jacobi_Uv( PolyMesh* pmesh, Eigen::MatrixX2d & uv, Eigen::MatrixXd & local_cord, Eigen::Matrix2d & P, Eigen::Matrix2d & S, Eigen::Matrix2d & J, std::vector<Eigen::Matrix2d>& l_t) {
    size_t face_num = pmesh->numPolygons();
...
void Tutte_Embedding(PolyMesh *mesh)
{
    ...
    SMatrix coff(n_vertices - boundary_nums,n_vertices - boundary_nums);
            ...
            for(size_t j = 0; j < 3; j++){
                ...
                switch (j) {
                    case 0:
                        ...
                        break;
                    case 1:
                        local_edge << local_cord(face_index, 4) - local_cord(face_index, 2), local_cord(face_index, 5) - local_cord(face_index, 3) ;
                        break;
                    case 2:
